...
def test_search_returns_results_shape_and_logs(tmp_path: Path, monkeypatch) -> None:
    ...
    async def fake_search_videos(query: str, max_results: int = 12, client=None):
        ...
        assert query == "cats"
        return [{"video_id": "abc123xyz99", "title": "Cats", "channel_id": "UC123", "channel_title": "CatTV", "thumbnail_url": "https://img", "published_at": "2024-01-01T00:00:00Z", "duration_seconds": 50, "is_short": True}]
...
def test_category_delete_or_archive_when_used(tmp_path: Path) -> None:
    ...
    with Session(engine) as session:
        session.execute(text("INSERT INTO categories(name, enabled) VALUES ('Used', 1), ('Unused', 1)"))
        session.execute(text("INSERT INTO channels(youtube_id, title, category_id, resolve_status) VALUES ('UCX', 'X', 1, 'ok')"))
        session.commit()
...
def test_recent_logs_include_kid_name(tmp_path: Path) -> None:
    ...
    with Session(engine) as session:
        ...
        session.execute(text("INSERT INTO channels(youtube_id, title, resolve_status, allowed) VALUES ('UC1', 'ABC', 'ok', 1)"))
        session.execute(text("INSERT INTO videos(youtube_id, channel_id, title, thumbnail_url, published_at) VALUES ('vid12345678A', 1, 'Hi', 'https://img', '2024-01-01T00:00:00Z')"))
        session.execute(text("INSERT INTO watch_log(kid_id, video_id, seconds_watched, created_at) VALUES (1, 1, 40, '2024-01-01T01:00:00Z')"))
        session.commit()
...
def test_schedule_sunday_first_enforcement(tmp_path: Path) -> None:
    ...
    with Session(engine) as session:
        ...
        session.execute(text("INSERT INTO kid_schedules(kid_id, day_of_week, start_time, end_time) VALUES (1, 0, '09:00', '10:00')"))
        session.commit()

        sunday = datetime(2024, 6, 2, 9, 30, tzinfo=timezone.utc)
        monday = datetime(2024, 6, 3, 9, 30, tzinfo=timezone.utc)
        assert is_in_any_schedule(session, kid_id=1, now=sunday) is True
        assert is_in_any_schedule(session, kid_id=1, now=monday) is True  # no monday windows => unrestricted
